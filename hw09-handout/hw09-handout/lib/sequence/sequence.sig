...
(* added ic *)
  (* exists p s ==> b
     ENSURES: b = true iff s contains an element x such that p x == true
   *)
  val exists: ('a -> bool) -> 'a seq -> bool
  (* all p s ==> b
     ENSURES: b = true iff all elements x in s are such that p x == true
   *)
  val all: ('a -> bool) -> 'a seq -> bool
  (* eq elem_eq (s1,s2) ==> b
     ENSURES: b = true iff s1 and s2 are equal where elements are compared
              using elem_eq
   *)
  val eq: ('a * 'a -> bool) -> 'a seq * 'a seq -> bool
  (* permeq elem_eq (s1,s2) ==> b
     ENSURES: b = true iff s1 and s2 contain the same elements in the same
              numbers (but possibly in different order) where elements are
              compared using elem_eq
   *)
  val permeq: ('a * 'a -> bool) -> 'a seq * 'a seq -> bool
  (* seq s ==> l
     ENSURES: l is the order-preserving list of the elements in s
   *)
